...
const StudentEvents = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const allEvents = await getEventsForAudience('students');
  ...
  const filteredEvents = events.filter((event) => {
    // Filter by target audience - students should see events for "all" or "students"
    const targetAudience = event.targetAudience || 'all';
    const isVisibleToStudent = targetAudience === 'all' || targetAudience === 'students';
    
    if (!isVisibleToStudent) return false;
