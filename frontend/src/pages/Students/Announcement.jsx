...
const StudentAnnouncements = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      setError("Không th? t?i thông báo. Vui lòng ki?m tra l?i k?t n?i Server.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatDate = (dateValue) => {
    if (!dateValue) return "G?n dây";
    try {
      const date = new Date(dateValue);
      if (isNaN(date.getTime())) return "G?n dây";
    ...
    <div className="announcements-page">
      <div className="page-header">
        <div className="header-title">
          ...
          <h1>Thông báo t? Nhà tru?ng</h1>
        </div>
        <span className="announcement-count">
          {announcements.length} thông báo m?i
        </span>
      </div>
      ...
      {error && (
        <div className="error-state">
          ...
          <p>{error}</p>
          <button className="view-btn" onClick={fetchAnnouncements} style={{ marginTop: '1rem' }}>Th? l?i</button>
        </div>
      )}
      ...
      {!loading && !error && announcements.length === 0 && (
        <div className="empty-state">
          ...
          <h3>Chua có thông báo</h3>
          <p>Hi?n t?i chua có thông báo m?i nào dành cho {studentClassName || "b?n"}.</p>
        </div>
      )}
            ...
            >
              ...
              <div className="card-footer">
                <span className="target-classes">
                  Dành cho: {item.targetAudience === 'all' ? 'Toàn tru?ng' : 'Toàn b? H?c sinh'}
                </span>
