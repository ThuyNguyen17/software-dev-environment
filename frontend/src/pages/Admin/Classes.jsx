...
const Classes = () => {
    ...
    const handleDeleteClass = async (id) => {
        if (window.confirm('B?n có ch?c ch?n mu?n xóa l?p này?')) {
            try {
    ...
    return (
        <ClassesContainer>
            <Content isOpen={isOpen}>
                <ClassesContent>
                    <ClassHeader>Qu?n lý L?p h?c</ClassHeader>
                    <AddClassForm onSubmit={handleAddClass}>
                        ...
                        />
                        <AddClassButton type="submit">Thêm l?p</AddClassButton>
                    </AddClassForm>
                    <ClassList>
                        {classes.map((cls) => (
                            <ClassItem key={cls.id}>
                                {editingClass === cls.id ? (
                                    <form onSubmit={(e) => handleUpdateClass(e, cls.id)}>
                                        ...
                                        />
                                        <ActionButtons>
                                            <AddClassButton type="submit">Luu</AddClassButton>
                                    ...
                                    <>
                                        <h3>{cls.grade}</h3>
                                        <ActionButtons>
                                            <EditButton onClick={() => startEdit(cls)}>S?a</EditButton>
